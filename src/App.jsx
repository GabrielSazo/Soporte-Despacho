      ...
      <section className="ticket-detail-modal" role="dialog" aria-modal="true" aria-labelledby="ticket-detail-title" onMouseDown={(event) => event.stopPropagation()} onPaste={handleAttachPaste}>
        ...
        {isLoading ? <div className="detail-loading"><span className="loading-mark"><i /><i /><i /></span><p>Cargando historial del ticket...</p></div> : <div className="ticket-detail-content">
          <div className="detail-main">
            ...
            {ticket.resolutionNotes && <section className="detail-section solution-detail"><span className="detail-label"><Icon name="checkCircle" size={15} /> Solución registrada</span><p>{ticket.resolutionNotes}</p>{ticket.attachments?.filter((a) => a.kind === "SOLUCION").length > 0 && <div className="solution-evidence"><span>Evidencia de solución · {ticket.attachments.filter((a) => a.kind === "SOLUCION").length} imagen{ticket.attachments.filter((a) => a.kind === "SOLUCION").length === 1 ? "" : "es"}</span><div className="solution-thumbs">{ticket.attachments.filter((a) => a.kind === "SOLUCION").map((a) => <a key={a.id} href={a.url} target="_blank" rel="noreferrer" title={a.original_name}><img src={a.url} alt={a.original_name} loading="lazy" onError={(e) => { e.target.style.display = "none"; }} /></a>)}</div></div>}</section>}
